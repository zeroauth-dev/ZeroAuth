...
From an external host: `nmap -p 1-65535 104.207.143.14 | grep open` should return exactly `22/tcp open`, `80/tcp open`, `443/tcp open`. `curl -sI https://api.zeroauth.dev/api/health` returns 200. `curl -sI https://api.zeroauth.dev/api/auth/saml/callback` in production returns 404 (the legacy demo-auth surface was removed in the June 2026 dead-API sweep).
