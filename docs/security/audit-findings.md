...
| **C-7** | Verifier loads `verification_key.json` from disk without checking it matches the circuit version compiled in code | **OPEN — sprint 2** | — | ADR 0015 landed at commit `27ed93c`. Implementation (boot-time SHA-256 check on `verification_key.json`) tracked as C-018. Test: `tests/zkp-version.test.ts`. |
...
| **C-12** | No cross-tenant rejection test matrix; tenant isolation relies on each developer remembering to add the right `WHERE` clause | **OPEN — week 2** | — | Tracked as C-007. Express introspection-driven test enumerates every `/v1/*` route. |
