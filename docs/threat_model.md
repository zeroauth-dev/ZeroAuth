...
| **Payment step-up** | The same pin protects high-value payments: a transfer ≥ ₹10,000 opens a pinned, `context_label`-tagged session, and money moves (`commitTransferIfApproved`) ONLY when that session reaches `consumed` — i.e. the account holder's own face approved it. The gate **owns** the invariant rather than inheriting it: it re-asserts, before debiting, that the linked session is a *payment* session (has a `context_label`) pinned to *this account's* DID (`expected_did = account.did`) — a consumed login session, an unlabelled session, or one pinned to another DID is declined, never settled. The debit is a single-winner status-flip + a funds-guarded `UPDATE`, so it is idempotent and cannot double-spend or overdraw. Amounts are safe-integer-bounded (≤ ₹10cr) so the consent label + audit can't lose precision. A wrong face → pin reject → session stays `issued` → the transfer is never committed and expires to `declined`. `tests/demo-bank-ledger.test.ts` pins the money-gate (consumed→debit, issued→no-debit, expired/failed→declined, wrong-DID→declined, unlabelled→declined, idempotent). |
