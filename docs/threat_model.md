...
| **Audit signal** | `pairing.pinned_did_mismatch` on a wrong-identity approval attempt (both `submitProof` and `verifyIdentityProof`); `bank.account_opened`, `bank.login_password_ok` on the bank flow. |
