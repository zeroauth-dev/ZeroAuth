...
| `POST` | `/v1/identity/challenge` | `zkp:verify` | **Issue a single-use verify challenge (A-02).** Returns `201 { challengeId, nonce, expiresAt }`. The on-device prover folds `nonce` into the proof as `publicSignals[1] = Poseidon(didHash, nonce)`. `429 too_many_pending_challenges` over the open-challenge cap. |
| `POST` | `/v1/identity/verify` | `zkp:verify` | **Face-first verify (replay-safe, A-02).** Accepts `{ did, challengeId, proof, publicSignals }`. Reuses the hardened proof-pairing verifier: DID lookup, constant-time commitment compare (`publicSignals[0]`), nonce binding (`publicSignals[1]` vs server-recomputed `Poseidon(didHash, nonce)`), Groth16 verify against the boot-pinned vkey, and **single-use consume** of the challenge — so a captured proof cannot be replayed. `200 { accessToken, refreshToken, sessionId, did }`; uniform `401 verification_failed` for did_unknown/commitment/nonce/proof (enumeration defence); `404 challenge_not_found`; `409 challenge_already_used`; `410 challenge_expired`. |
...
| `POST` | `/api/demo-portal/device/pending` | `{ did }` → the app's approval inbox: issued sessions pinned to that DID, each carrying the same `za:pair:1:` challenge a QR would (`requests[]`), plus a `contextLabel` + `kind` (`login` \| `payment`) so the app can render "Login approval" vs "Payment approval · ₹X to Y". Approval still requires the enrolled face (A-47). |
| `GET` | `/api/demo-portal/bank/overview` | Cookie-authed dashboard payload: `{ fullName, primaryBalanceDisplay, accounts[], transactions[], stepUpThresholdDisplay }`. Ledger is seeded on first read. `401` / `404 no_account`. |
| `POST` | `/api/demo-portal/bank/transfer` | Cookie-authed. `{ amount (rupees), payeeName, note? }`. **< ₹10,000 settles immediately** (`200 { requiresApproval:false, balanceDisplay }`); **≥ ₹10,000 opens a DID-pinned, labelled "Payment approval" session** (`201 { requiresApproval:true, transferId, sessionId, qrPayload, contextLabel }`) — money moves only when the account holder's face consumes it. `400 invalid_request/insufficient_funds`. |
| `GET` | `/api/demo-portal/bank/transfer/:id` | Cookie-authed poll. Settles the step-up transfer iff its pinned session is `consumed`. `200 { status: pending_approval\|completed\|declined, balanceDisplay? }`. Idempotent. |
