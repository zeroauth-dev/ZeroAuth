/*
 * poseidon.js — Browser-safe Poseidon2 hash for the W3 phone-side fold.
 *
 * Pinned to the same parameters as `poseidon-lite@0.3.0` (npm) which is
 * the package used on both the server (`src/services/identity.ts`) and
 * the IoT bridge (`iot/src/crypto.ts`). The MIT-licensed poseidon-lite
 * source ships as CommonJS; we inline the poseidon2 round-constants and
 * MDS matrix below so the WebView's `'script-src self'` CSP can load
 * this file as plain ES5 with zero network access, zero eval, no
 * dynamic require.
 *
 * Exports a single global: `window.zaPoseidon2(a, b)` returning a
 * decimal-string BN128 scalar. Matches `poseidon2([a, b])` from the
 * npm package over BN128's scalar field
 * (21888242871839275222246405745257275088548364400416034343698204186575808495617).
 *
 * Used by prover.js for the Option B' fold:
 *   didHashSession  = poseidon2(didHashRaw, sessionNonce)
 *   identityBinding = poseidon2(biometricSecret, didHashSession)
 *
 * See ADR-0009 §"Decision/Protocol (Option B')" + ADR-0010.
 */

(function () {
  'use strict';

  // BN128 scalar field. Same modulus snarkjs / circomlib / poseidon-lite
  // operate over. Hex form for readability; we BigInt-decode at load.
  var F = BigInt(
    '21888242871839275222246405745257275088548364400416034343698204186575808495617'
  );

  // poseidon-lite/constants/2.js, base64-decoded big-endian per
  // poseidon-lite's `unstringify.js`. Round constants C (267 entries)
  // and MDS matrix M (3 x 3) for t = 3 (two inputs + capacity).
  var C_B64 = [
  ...
  // poseidon-lite's `unstringify.js` decodes each base64 string as a
  // big-endian byte array and reads it as a BigInt. Match exactly.
  ...
  var C = C_B64.map(b64ToBigInt);
  var M = M_B64.map(function (row) {
    return row.map(b64ToBigInt);
  });

  // Poseidon parameters for t=3 (two inputs + capacity).
  var N_ROUNDS_F = 8;
  var N_ROUNDS_P = 57; // table 8 of the Poseidon whitepaper, t=3.
  ...
  /**
   * poseidon2(a, b) → BigInt
   *
   * Pure-function implementation mirroring `poseidon-lite/poseidon2.js`
   * for inputs.length === 2.
   */
  function poseidon2(a, b) {
    var inputs = [BigInt(a), BigInt(b)];
    var state = [0n, inputs[0], inputs[1]];
    var t = 3;
    var total = N_ROUNDS_F + N_ROUNDS_P;
    for (var x = 0; x < total; x++) {
      for (var y = 0; y < t; y++) {
        state[y] = (state[y] + C[x * t + y]) % F;
    ...
    // BigInt → ensure non-negative ((a%b+b)%b shape for safety).
    var out = state[0] % F;
    if (out < 0n) out = out + F;
    return out;
  }

  // Decimal string is what snarkjs's fullProve consumes for witness
  // field elements. Expose both flavors so prover.js can pick whichever
  // it needs without re-stringifying.
  function poseidon2Decimal(a, b) {
    return poseidon2(a, b).toString(10);
  }

  // Exposed on `window` so prover.js (loaded as a separate <script>) can
  // reach it. No ES modules — we want to stay in the WebView's strict
  // CSP, no `import.meta`, no `type=module`.
  window.zaPoseidon2 = poseidon2;
  window.zaPoseidon2Decimal = poseidon2Decimal;
})();
